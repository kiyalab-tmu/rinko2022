...
## Input Layer（参考書籍→p.34~）
* Input Layerでは，主に４つ処理を行いますが，それらは何であるか調べましょう（参考書籍の2-3を写して構いません）

### 2-3-1 パッチに分割（参考書籍→p.35~）
* パッチ分割とFlattenについて説明してください（なぜパッチ分割・ベクトル化が必要なのかなども含めて）

### 2-3-2 パッチの埋め込み（参考書籍→p.38~）
...
* 必要であれば，参考書籍の力作の４コマ漫画の例を引用しても構いません
* Self-Attentionの全体像を簡単に説明してください

### 2-4-2 Self-Attentionの埋め込み（参考書籍→p.50~）
* Query，Key，Valueはどのようにして使われるか説明してください

### 2-4-3 Self-Attentionの内積（参考書籍→p.51~）
